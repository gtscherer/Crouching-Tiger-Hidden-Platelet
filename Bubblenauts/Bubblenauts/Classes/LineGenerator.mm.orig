...
bool LineGenerator::requiredEntityBelow(char required, int position)
{
    ...
    {
        Line::iterator previousLineEntity = this->previousLine.begin();
...
StringRepresentation LineGenerator::getNextLine()
{
    ...
    Line::iterator linePosition = workingLine.begin();
    ...
    for(Probability_To_Generate probability : secondaryProbabilityValues)
    {
        if(this->checkIfBlocked(*linePosition))
        {
            if(*linePosition == Air){
                ...
                {
                    if(previousEntity == Platform && EntityProbabilityPair.first == Platform)
                    {
                        Probability_To_Generate platformProbability = EntityProbabilityPair.second + ClusteringFactor;
                        if(probability < platformProbability && probability > BubbleProbability && probability > SudsProbability) *linePosition = EntityProbabilityPair.first;
                        else if (probability < platformProbability) //Finish what you were doing
                    }
                    else{
                        if(probability < PlatformProbability)
                        {
